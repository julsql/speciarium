...
export type UploadStage = 'idle' | 'scanning' | 'confirm' | 'uploading' | 'done' | 'error';
...
export interface UploadState {
  ...
  error?: string;
  /** Pending state when stage='confirm', awaiting user approval */
  pending?: {
    files: File[];
    metadata: FileMeta[];
    toDelete: string[];
...
interface UploadContextValue {
  state: UploadState;
  /** Open file picker → scan → prepare → set stage='confirm'. */
  pickFolder: (collectionId: number) => void;
  /** Start the actual upload once the user has confirmed. Runs in background. */
  confirm: () => Promise<void>;
  /** Cancel a pending confirmation. */
  cancel: () => void;
  /** Reset to idle after done/error so the next upload starts clean. */
  reset: () => void;
  /**
   * Compteur incrémenté à chaque fin d'upload réussi. Les pages peuvent
   * l'inclure dans leurs dépendances useEffect pour re-fetch automatiquement
   * leurs données après un import.
   */
  dataVersion: number;
}
...
export function BackgroundUploadProvider({ children }: { children: ReactNode }) {
  ...
  const pendingRef = useRef<UploadState['pending'] | null>(null);
  const collectionIdRef = useRef<number | null>(null);
  const inputRef = useRef<HTMLInputElement | null>(null);

  const pickFolder = useCallback((collectionId: number) => {
    if (!inputRef.current) {
  ...
  function onFileInputChange(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!files || files.length === 0 || collectionId == null) return;
    scan(collectionId, files);
  }

  async function scan(collectionId: number, rawList: FileList) {
    setState({
  ...
  const confirm = useCallback(async () => {
    ...
    const collectionId = collectionIdRef.current;
    if (!pending || collectionId == null) return;
    setState({
  ...
  async function doUpload(
    pending: NonNullable<UploadState['pending']>,
    collectionId: number,
  ) {
    ...
    function markDone(failed: boolean, errorMsg?: string) {
      if (finished) return;
      finished = true;
    ...
    try {
      for (let i = 0; i < files.length; i += CHUNK_SIZE) {
        ...
        if (i === 0) form.append('imageToDelete', JSON.stringify(toDelete));
        ...
        if (!resp.ok) {
          ...
          try {
            ...
            if (body?.error) msg = body.message ? `${body.error} — ${body.message}` : body.error;
  ...
  return (
    <Ctx.Provider value={{ state, pickFolder, confirm, cancel, reset, dataVersion }}>
      {/* Input caché — monté une fois, dans le tree React, pour que l'event
          onChange soit fiable et que le picker s'ouvre dès le premier clic. */}
      <input
        ref={inputRef}
        type="file"
        multiple
...
export function useBackgroundUpload() {
  ...
  if (!ctx) throw new Error('useBackgroundUpload doit être utilisé sous BackgroundUploadProvider');
  return ctx;
}
...
function parseExifDateTime(view: DataView, tiffStart: number, _length: number): string | null {
